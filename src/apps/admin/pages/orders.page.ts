    ...
    async selectFirstShippingMethodToAddToOrder() {
        await test.step(
            ...
            async () => {
                ...
                // Wait for shipping method radio buttons to appear
                const shippingMethod = this.page.locator('input[name="order[shipping_method]"]').first();
                await shippingMethod.waitFor({ state: 'visible', timeout: 15000 });
